...
# NOTE: Désactivée car les données sont déjà montées via Docker volume
# Si vous voulez utiliser DVC, initialisez-le d'abord dans le conteneur
...
train_model_task = PythonOperator(
    ...
    op_kwargs={
        ...
        'epochs': 1,  # 1 seul epoch pour démo rapide
        'batch_size': 64,  # Batch plus gros = plus rapide
        'learning_rate': 0.001,
        'sample_fraction': 0.1,  # Utiliser seulement 10% des données pour test ultra-rapide
    },
    dag=dag,
)
...
# Données déjà disponibles, on commence directement par l'entraînement
